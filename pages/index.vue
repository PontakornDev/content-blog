<template>
  <div>
    <main>
      <section class="lg:px-[15%] px-[5%] pt-20">
        <h1
          class="lg:text-5xl text-2xl leading-normal font-semibold text-center py-4"
        >
          Check Out Our Latest Blog Posts
        </h1>
        <h2
          class="lg:text-2xl text-xl text-center leading-normal font-bold rainbow-text"
        >
          Web blog to Learning and Development
        </h2>
      </section>
      <section class="lg:px-[15%] px-[5%] lg:pt-20 pt-14">
        <p
          class="text-center uppercase font-semibold tracking-wider mb-10 text-gray-500"
        >
          Blog Posts
        </p>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div
            class="px-7 py-5 rounded-lg border-2 border-amber-700"
            v-for="(b, i) in blogNav[0].children"
            :key="`blogNavItem-${b._path}-${i}`"
          >
            <h2 class="text-lg font-semibold rainbow-text">
              {{ b.title }}
            </h2>
            <!-- Loop over files inside the content dir -->
            <ul
              v-if="b.children"
              class="list-disc list-inside mt-4 pl-2 space-y-3"
            >
              <li
                class="list-item text-sm text-gray-600 hover:text-primary-900 underline underline-offset-4 decoration-wavy decoration-primary/40 hover:decoration-primary transition-all"
                v-for="(child, k) in b.children"
                :key="`childNav-${child._path}-${k}-${i}`"
              >
                <NuxtLink :to="`/blog${child._path}`">
                  {{ child.title }}
                </NuxtLink>
              </li>
            </ul>
            <ul v-else class="list-disc list-inside mt-4 pl-2 space-y-3">
              <li
                class="list-item text-sm text-gray-600 hover:text-primary-900 underline underline-offset-4 decoration-wavy decoration-primary/40 hover:decoration-primary transition-all"
              >
                <NuxtLink :to="`/blog${b._path}`"> Get Started </NuxtLink>
              </li>
            </ul>
          </div>
        </div>
      </section>
    </main>
  </div>
</template>

<script setup>
const { data: blogNav } = await useAsyncData("navigation", () => {
  return fetchContentNavigation(queryContent("blog"));
});
useHead({
  title: "Content Blog",
  meta: [
    {
      name: "description",
      content: "Blog แชร์ประสบการณ์ในการพัฒนาเว็บไซต์ต่างๆ",
    },
    { property: "og:title", content: "DEV PAI - Content Blog" },
    {
      property: "og:description",
      content: "Blog แชร์ประสบการณ์ในการพัฒนาเว็บไซต์ต่างๆ",
    },
    { property: "og:locale", content: "th_TH" },
    { property: "og:type", content: "website" },
    {
      property: "og:image",
      content: "https://www.devpai.netlify.app/icon.png",
    },
    { property: "og:image:width", content: "500" },
    { property: "og:image:height", content: "500" },
    { property: "og:image:alt", content: "DevLearning/website " },
    { property: "og:url", content: "https://www.devpai.netlify.app" },
    { property: "twitter:title", content: "DEV PAI - Content Blog" },
    {
      property: "twitter:description",
      content: "Blog แชร์ประสบการณ์ในการพัฒนาเว็บไซต์ต่างๆ",
    },
    { property: "twitter:locale", content: "th_TH" },
    { property: "twitter:type", content: "website" },
    {
      property: "twitter:image",
      content: "https://www.devpai.netlify.app/icon.png",
    },
    { property: "twitter:image:width", content: "500" },
    { property: "twitter:image:height", content: "500" },
    { property: "twitter:image:alt", content: "DevLearning/website " },
    { property: "twitter:url", content: "https://www.devpai.netlify.app" },
  ],
  link: [{ rel: "canonical", href: "https://www.devpai.netlify.app" }],
});
</script>
